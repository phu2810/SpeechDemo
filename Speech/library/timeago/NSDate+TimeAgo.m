...
{
    NSDate *now = [NSDate date];
    now = [NSDate dateWithTimeInterval:7*60*60 sinceDate:now];
    double deltaSeconds = fabs([self timeIntervalSinceDate:now]);
    double deltaMinutes = deltaSeconds / 60.0f;
    ...
    if(deltaSeconds < 5)
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"Just now"]);
    }
    else if(deltaSeconds < 60)
    {
        return [self stringFromFormat:[self localizedString:@"%%d %@seconds ago"] withValue:deltaSeconds];
    }
    else if(deltaSeconds < 120)
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"A minute ago"]);
    }
    else if (deltaMinutes < 60)
    {
        return [self stringFromFormat:[self localizedString:@"%%d %@minutes ago"] withValue:deltaMinutes];
    }
    else if (deltaMinutes < 120)
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"An hour ago"]);
    }
    else if (deltaMinutes < (24 * 60))
    {
        minutes = (int)floor(deltaMinutes/60);
        return [self stringFromFormat:[self localizedString:@"%%d %@hours ago"] withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 2))
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"Yesterday"]);
    }
    else if (deltaMinutes < (24 * 60 * 7))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24));
        return [self stringFromFormat:[self localizedString:@"%%d %@days ago"] withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 14))
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"Last week"]);
    }
    else if (deltaMinutes < (24 * 60 * 31))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24 * 7));
        return [self stringFromFormat:[self localizedString:@"%%d %@weeks ago"] withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 61))
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"Last month"]);
    }
    else if (deltaMinutes < (24 * 60 * 365.25))
    {
        minutes = (int)floor(deltaMinutes/(60 * 24 * 30));
        return [self stringFromFormat:[self localizedString:@"%%d %@months ago"] withValue:minutes];
    }
    else if (deltaMinutes < (24 * 60 * 731))
    {
        return NSDateTimeAgoLocalizedStrings([self localizedString:@"Last year"]);
    }
    
    minutes = (int)floor(deltaMinutes/(60 * 24 * 365));
    return [self stringFromFormat:[self localizedString:@"%%d %@years ago"] withValue:minutes];
}
