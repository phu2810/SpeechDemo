...
@interface MWPhoto () {
    ...
    id _webImageOperation;//id <SDWebImageOperation> _webImageOperation;
...
- (void)performLoadUnderlyingImageAndNotify {
    ...
    // Phunv: add case
    else if (self.imageNode) {
        [self updateProcessDownloadImage];
    }
    else {
...
// Phunv: them ham nay
- (void) updateProcessDownloadImage {
    if (self.imageNode) {
        UIImage * image = self.imageNode.image;
        if (image) {
            if (!self.underlyingImage) {
