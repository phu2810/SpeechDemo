...
// Phunv - function update progress
- (void) updateProgressPhoto {
    id photo = [self photoAtIndex:self.currentIndex];
    if ([photo isKindOfClass:[MWPhoto class]]) {
        MWPhoto *mwPhoto = photo;
        [mwPhoto updateProcessDownloadImage];
    }
}
- (void)dealloc {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    // Phunv - init timer
    self.timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(updateProgressPhoto) userInfo:nil repeats:YES];
}
